...
public class ElectronicActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        trackScreen("Electronic","ElectronicActivity");
    ...
    private void setOnClickListener(){
        listener = new myAdapter.RecyclerViewClickListener() {
            ...
            public void onClick(View v, int position) {
                if(productList.get(position).getName().equals("laptop hp")){
                    selectContent("12",productList.get(position).getName(),"text view");
                    saveTime(date,"Electronic Products");
                    Intent intent = new Intent(getApplicationContext(),ElectronicDetails.class);
                    intent.putExtra("electronicName",productList.get(position).getName());
                    startActivity(intent);
                }
                if(productList.get(position).getName().equals("iphone x")){
                    selectContent("13",productList.get(position).getName(),"text view");
                    saveTime(date,"Electronic Products");
                    Intent  intent = new Intent(getApplicationContext(),ElectronicDetails.class);
                    intent.putExtra("electronicName",productList.get(position).getName());
                    startActivity(intent);
                }
                if(productList.get(position).getName().equals("smart tv")){
                    selectContent("14",productList.get(position).getName(),"text view");
                    saveTime(date,"Electronic Products");
                    Intent  intent = new Intent(getApplicationContext(),ElectronicDetails.class);
                    intent.putExtra("electronicName",productList.get(position).getName());
                    startActivity(intent);
